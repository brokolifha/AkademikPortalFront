...
const dtFetchData = async () => {
  ...
  try {
    const response = await useApi.get("/juri/all");
...
const dtSaveRecord = async () => {
  if (editMode.value) {
    try {
      await useApi.put(`/users/juri/update/${dtRecord.value.id}`, dtRecord.value);
      isModalOpen.value = false;
      alert("Jüri kaydı başarıyla güncellendi.");
    } catch (error) {
      alert(error.response?.data?.error || 'Güncelleme yapılırken bir hata oluştu.');
    }
  } else {
    try {
      await useApi.post("/users/juriRegister", dtRecord.value);
      isModalOpen.value = false;
      alert("Jüri kaydı başarıyla oluşturuldu.");
    } catch (error) {
...
const dtDelete = (item) => {
  const confirmDelete = confirm(
    `Bu juri kaydını silmek istediğinize emin misiniz?`
  );
  if (confirmDelete) {
    useApi
      .delete(`/users/juri/delete/${item.id}`)
      .then(() => {
        dtData.value = dtData.value.filter((i) => i.id !== item.id);
        dtDataFiltered.value = dtDataFiltered.value.filter(
          (i) => i.id !== item.id
        );
        alert("Jüri kaydı başarıyla silindi.");
      })
      .catch((error) => {
...
<template>
  <v-dialog v-model="isModalOpen" max-width="600" persistent scrollable>
    <v-card>
      <v-card-title>
        <span class="text-h5">Jüri Kayıt</span>
      </v-card-title>
