...
const adayDegerlendirModal = ref(false);
const adayDegerlendirBasvuru = ref(null);
const adayDegerlendirForm = ref({
  basvuruId: null,
  puan: null,
  yorum: null,
});
...
const resetAdayDegerlendirForm = () => {
  adayDegerlendirForm.value.basvuruId = null;
  adayDegerlendirForm.value.puan = null;
  adayDegerlendirForm.value.yorum = null;
  adayDegerlendirBasvuru.value = null;
};
const adayDegerlendirOpenDialog = (basvuru) => {
  resetAdayDegerlendirForm();
  basvuru.adSoyad = basvuru.adayAdi + " " + basvuru.adaySoyadi;
  adayDegerlendirForm.value.basvuruId = basvuru.id;
  adayDegerlendirModal.value = true;
  adayDegerlendirBasvuru.value = basvuru;
//  adayDegerlendirForm.value.juriUyesiId = userData.value.id;
};

const adayDegerlendir = () => {
  if (!adayDegerlendirForm.value.puan) {
    alert("Lütfen puan giriniz.");
    return;
  }

  useApi
    .post('/juri/degerlendirme', adayDegerlendirForm.value)
    .then(({ data }) => {
      console.log("Aday değerlendirme sonucu:", data);
      adayDegerlendirModal.value = false;
      getData();
    })
    .catch((error) => {
      ...
      alert(
        error?.response?.data ||
          "Aday değerlendirme işlemi sırasında bir hata oluştu."
      );
    });
}
...
const getData = () => {
  ...
  let endpoint = "/basvurular/all";

  if (userData.value.rol == "juri") {
    endpoint = `/juri/basvurular`;
  }
  ...
  useApi
    ...
    .then(({ data }) => {
      data.map((basvuru) => {
        basvuru.ilanBasligi = basvuru.ilanId + " - " + basvuru.ilanBasligi;
      });
      dtData.value = data;
  ...
  if (confirm("Bu jüriyi silmek istediğinize emin misiniz?")) {
    useApi
      ...
      .catch(({ response }) => {
        ...
        alert(
          response.data?.error || "Jüri silme işlemi sırasında bir hata oluştu."
        );
      })
      .finally(() => {
        atananJuriModal.value = false;
...
const ilanaJuriAta = () => {
  ...
  useApi
    .post(`/juri/atama?ilanId=${juriAtaIlanId.value}`, juriAtaSelected.value)
    .then(({ data }) => {
...
const basvuruDegerlendirmeListesiGetir = (basvuruId) => {
  useApi
    .get(`/yonetici/basvuru/degerlendirmeler/${basvuruId}`)
    .then(({ data }) => {
      console.log("Değerlendirme Listesi:", data);

      data.map((degerlendirme) => {
        let juri = juriAtaData.value.find(
          (j) => j.id == degerlendirme.juriUyesiId
        );
    ...
    .catch((error) => {
      ...
      alert(
        error?.response?.data?.error ||
          "Nihai karar verme işlemi sırasında bir hata oluştu."
      );
    });
};
...
const basvuruBelgeOlustur = (basvuru) => {
  useApi
    .get("/pdf/tablo5/" + basvuru.id, { responseType: "blob" })
    .then(({ data }) => {
    ...
    .catch((error) => {
      ...
      alert(
        error?.response?.data?.error ||
          "Belge oluşturma işlemi sırasında bir hata oluştu."
      );
    });
};
...
const basvuruJuriBelgeleriniIndir = (basvuruId) => {
  useApi
    .get(`/juri/basvuru/belgeler/${basvuruId}`, { responseType: "blob" })
    .then(({ data }) => {
...
// Juri fonksiyonları
const juriAdayDegerlendir = (basvuru) => {
...
watch(basvuruDurumuFiltre, (newValue) => {
  if (newValue) {
    dtDataFiltered.value = dtData.value.filter(
      (item) => item.basvuruDurumu === newValue
    );
  } else {
...
<template>
        ...
        <v-list two-line>
          <v-list-item v-for="j in atananJuriler" :key="j.id">
            ...
            <template v-slot:append>
              <v-btn
                icon
                @click="ilandanJuriSil(juriAtaIlanId, j.id)"
                variant="text"
                color="error"
  ...
  <v-dialog v-model="basvuruDegerlendirmeModal" max-width="70%">
    <v-card>
      ...
      <v-card-text>
        <v-list three-line>
          <template v-for="b in basvuruDegerlendirmeListesi" :key="b.id">
            <v-list-item>
              <v-list-item-title
                >{{ b.juri.ad }} {{ b.juri.soyad }} <v-spacer />
              </v-list-item-title>
              <v-list-item-subtitle>
                {{ formatDate(b.degerlendirmeTarihi) }} - Puan:
                <strong>{{ b.puan }}</strong>
              </v-list-item-subtitle>
              <v-list-item-subtitle>
                {{ b.yorum }}
              </v-list-item-subtitle>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn text @click="basvuruDegerlendirmeModal = false">Kapat</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  <v-dialog v-model="adayDegerlendirModal" max-width="500">
    <v-card>
      <v-card-title>Aday Değerlendir - {{ adayDegerlendirBasvuru?.adSoyad }}</v-card-title>
      <v-card-text>
        <v-text-field
          v-model="adayDegerlendirForm.puan"
          label="Puan"
          type="number"
          min="0"
          max="100"
        ...
        <v-textarea
          v-model="adayDegerlendirForm.yorum"
          label="Yorum"
          rows="3"
          class="mt-4"
        ></v-textarea>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn text @click="adayDegerlendirModal = false">Kapat</v-btn>
        <v-btn text @click="adayDegerlendir()">Kaydet</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  <v-card>
    ...
    <v-card-text>
      ...
      >
          ...
          <tr>
            <td :colspan="columns.length">
              <div class="d-flex align-center">
                ...
                <span class="ms-4">İlan: {{ item.value }} </span> <v-spacer />
                <span class="ms-4"
                  >Başvuru Sayısı: {{ item.items.length }}</span
                >

                <v-tooltip :open-delay="300" :close-delay="200"
                  v-if="userData.rol == 'yonetici'"
                >
                  <template #activator="{ props }">
                    <v-btn
                      ...
                      icon
                      variant="text"
                      @click="ilanaJuriAtaSecim(item.items[0].raw.ilanId)"
                    >
                      <v-icon> tabler-user-share </v-icon>
                    </v-btn>
                  </template>
                  Jüri Ata
                </v-tooltip>
                ...
                <v-tooltip
                  :open-delay="300"
                  :close-delay="200"
                  v-if="userData.rol != 'juri' && userData.rol != 'aday'"
                >
                  <template #activator="{ props }">
                    <v-btn
                      v-bind="props"
                      icon
                      variant="text"
                      @click="ilanaAtananJuriListesi(item.items[0].raw.ilanId)"
                    >
                      <v-icon> tabler-users </v-icon>
                    </v-btn>
                  </template>
                  Jürileri Görüntüle
                </v-tooltip>
        ...
        <template v-slot:item.actions="{ item }">
          <!-- Jüri İşlemleri -->
          <template v-if="userData.rol == 'juri'">
            <v-tooltip :open-delay="300" :close-delay="200">
              <template #activator="{ props }">
                <v-btn
                  v-bind="props"
                  ...
                  variant="text"
                  color="success"
                  @click="adayDegerlendirOpenDialog(item)"
                >
                  <v-icon> tabler-notes </v-icon>
                </v-btn>
              </template>
              Aday Değerlendir
            </v-tooltip>
          ...
          >
            <template #activator="{ props }">
              ...
              >
                <v-icon> tabler-file-text-spark </v-icon>
              </v-btn>
            </template>
            Belge Oluştur
          </v-tooltip>
          ...
          <v-tooltip
            ...
            :close-delay="200"
            v-if="
              item.basvuruDurumu != 'BEKLEMEDE' && userData.rol == 'yonetici'
            "
          >
            <template #activator="{ props }">
              <v-btn
              ...
              >
                <v-icon> tabler-file-type-zip </v-icon>
              </v-btn>
            </template>
          ...
          >
            <template #activator="{ props }">
              ...
              >
                <v-icon> tabler-message-user </v-icon>
              </v-btn>
            </template>
            Değerlendirmeler
          </v-tooltip>
          ...
          <v-tooltip
            ...
            :close-delay="200"
            v-if="
              (item.basvuruDurumu == 'DEGERLENDIRME_TAMAMLANDI' ||
                item.basvuruDurumu == 'KABUL_EDILDI' ||
                item.basvuruDurumu == 'RED_EDILDI') &&
